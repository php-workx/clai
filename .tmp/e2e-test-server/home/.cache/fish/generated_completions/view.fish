...
complete -c view -o dev -d 'Open {device} for use as a terminal.  Only on the Amiga'
...
complete -c view -s P -d 'Win32 GUI only: Specify the title of the parent application'
...
complete -c view -s x -d 'If  Vim has been compiled with encryption support, use encryption when writin…'
complete -c view -s X -d 'Don\'t connect to the X server'
complete -c view -s Y -d 'Don\'t connect to the wayland compositor'
...
complete -c view -l gui-dialog-file -d 'When using the GUI, instead of showing a dialog, write the title and message …'
complete -c view -l help -s h -s '?' -d 'Give a bit of help about the command line arguments and options'
complete -c view -l literal -d 'Take file name arguments literally, do not expand wildcards'
complete -c view -l log -d 'If  Vim has been compiled with eval and channel feature, start logging and wr…'
complete -c view -l nofork -d Foreground
complete -c view -l noplugin -d 'Skip loading plugins.   Implied by -u NONE'
complete -c view -l not-a-term -d 'Tells  Vim that the user knows that the input and/or output is not connected …'
...
complete -c view -l clientserver -d 'Use {backend} as the backend for clientserver functionality, either "socket" …'
...
complete -c view -l ttyfail -d 'When stdin or stdout is not a a terminal (tty) then exit right away'
...
complete -c view -l windowid -d 'Win32 GUI only: Make gVim try to use the window {id} as a parent, so that it …'
