...
complete -c llvm-profdata -l binary-file -d 'Specify the executable that contains profile data and profile name sections f…'
...
complete -c llvm-profdata -l function -d 'Only keep functions matching the regex in the output, all others are erased f…'
complete -c llvm-profdata -l no-function -d 'Remove functions matching the regex from the profile.  If both --function and'
