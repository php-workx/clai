# xz
# Autogenerated from man page /opt/homebrew/share/man/man1/xz.1
...
complete -c xz -l no-sync -d 'Do not synchronize the target file and its directory to the storage device be…'
...
complete -c xz -l filters -d 'Specify the full filter chain or a preset in a single option'
complete -c xz -l filters1 -l filters9 -d 'Specify up to nine additional filter chains that can be used with  --block-li…'
complete -c xz -l filters-help -d 'Display a help message describing how to specify presets and custom filter ch…'
complete -c xz -l lzma2 -d 'PD Add LZMA1 or LZMA2 filter to the filter chain'
complete -c xz -l riscv -d 'PD Add a branch/call/jump (BCJ) filter to the filter chain'
...
complete -c xz -l filters3 -d 'and 2 MiB input o 3 The filter chain specified by'
complete -c xz -l filters2 -d 'and 4 MiB input o 3 The filter chain specified by'
