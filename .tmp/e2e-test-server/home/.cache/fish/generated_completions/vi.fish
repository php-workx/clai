...
complete -c vi -o dev -d 'Open {device} for use as a terminal.  Only on the Amiga'
...
complete -c vi -s P -d 'Win32 GUI only: Specify the title of the parent application'
...
complete -c vi -s x -d 'If  Vim has been compiled with encryption support, use encryption when writin…'
complete -c vi -s X -d 'Don\'t connect to the X server'
complete -c vi -s Y -d 'Don\'t connect to the wayland compositor'
...
complete -c vi -l gui-dialog-file -d 'When using the GUI, instead of showing a dialog, write the title and message …'
complete -c vi -l help -s h -s '?' -d 'Give a bit of help about the command line arguments and options'
complete -c vi -l literal -d 'Take file name arguments literally, do not expand wildcards'
complete -c vi -l log -d 'If  Vim has been compiled with eval and channel feature, start logging and wr…'
complete -c vi -l nofork -d Foreground
complete -c vi -l noplugin -d 'Skip loading plugins.   Implied by -u NONE'
complete -c vi -l not-a-term -d 'Tells  Vim that the user knows that the input and/or output is not connected …'
...
complete -c vi -l clientserver -d 'Use {backend} as the backend for clientserver functionality, either "socket" …'
...
complete -c vi -l ttyfail -d 'When stdin or stdout is not a a terminal (tty) then exit right away'
...
complete -c vi -l windowid -d 'Win32 GUI only: Make gVim try to use the window {id} as a parent, so that it …'
