...
complete -c brew -l file -d 'Read from or write to the Brewfile from this location'
complete -c brew -s g -l global -d 'Read from or write to the Brewfile from $HOMEBREW_BUNDLE_FILE_GLOBAL (if set)…'
complete -c brew -s v -l verbose -d 'install prints output from commands as they are run'
complete -c brew -l no-upgrade -d 'install does not run brew upgrade on outdated dependencies'
complete -c brew -l upgrade -d 'install runs brew upgrade on outdated dependencies, even if $HOMEBREW_BUNDLE_…'
complete -c brew -l upgrade-formulae -d 'install runs brew upgrade on any of these comma-separated formulae, even if $…'
complete -c brew -l install -d 'Run install before continuing to other operations, e. g.  exec'
complete -c brew -l services -d 'Temporarily start services while running the exec or sh command'
complete -c brew -s f -l force -d 'install runs with --force/--overwrite.  dump overwrites an existing Brewfile'
complete -c brew -l cleanup -d 'install performs cleanup operation, same as running cleanup --force'
complete -c brew -l all -d 'list all dependencies'
complete -c brew -l formula -d 'list, dump or cleanup Homebrew formula dependencies'
complete -c brew -l cask -d 'list, dump or cleanup Homebrew cask dependencies'
complete -c brew -l tap -d 'list, dump or cleanup Homebrew tap dependencies'
complete -c brew -l mas -d 'list or dump Mac App Store dependencies'
complete -c brew -l vscode -d 'list, dump or cleanup VSCode (and forks/variants) extensions'
complete -c brew -l go -d 'list or dump Go packages'
complete -c brew -l cargo -d 'list or dump Cargo packages'
complete -c brew -l flatpak -d 'list or dump Flatpak packages.  Note: Linux only'
complete -c brew -l no-vscode -d 'dump without VSCode (and forks/variants) extensions'
complete -c brew -l no-go -d 'dump without Go packages'
complete -c brew -l no-cargo -d 'dump without Cargo packages'
complete -c brew -l no-flatpak -d 'dump without Flatpak packages'
...
complete -c brew -l zap -d 'cleanup casks using the zap command instead of uninstall'
complete -c brew -l check -d 'Check that all dependencies in the Brewfile are installed before running exec…'
complete -c brew -l no-secrets -d 'Attempt to remove secrets from the environment before exec, sh, or env'
complete -c brew -l prune -d 'Remove all cache files older than specified days'
complete -c brew -s s -l scrub -d 'Scrub the cache, including downloads for even the latest versions'
...
complete -c brew -l include-implicit -d 'Include implicit dependencies used to download and unpack source files'
...
complete -c brew -l include-test -d 'Include :test dependencies for formula (non-recursive unless --graph or --tre…'
...
complete -c brew -l os -d 'Show dependencies for the given operating system'
complete -c brew -l arch -d 'Show dependencies for the given CPU architecture'
...
complete -c brew -l fetch-manifest -d 'Fetch GitHub Packages manifest for extra information when formula is not inst…'
complete -c brew -l json -d 'Print a JSON representation'
complete -c brew -l variations -d 'Include the variations hash in each formula[u2019]s JSON output'
complete -c brew -l sizes -d 'Show the size of installed formulae and casks'
...
complete -c brew -l ask -d 'Ask for confirmation before downloading and installing formulae'
...
complete -c brew -l skip-link -d 'Install but skip linking the keg into the prefix'
complete -c brew -l as-dependency -d 'Install but mark as installed as a dependency and not installed on request'
...
complete -c brew -l git -d 'Create a Git repository, useful for creating patches to the software'
...
complete -c brew -l poured-from-bottle -d 'List the formulae installed from a bottle'
complete -c brew -l built-from-source -d 'List the formulae compiled from source'
...
complete -c brew -l command -d 'Show options for the specified command.  outdated [options] [formula|cask '
...
complete -c brew -l syntax -d 'Syntax-check all of Homebrew[u2019]s Ruby files (if no tap is passed)'
complete -c brew -l no-simulate -d 'Don[u2019]t simulate other system configurations when checking formulae and c…'
...
complete -c brew -l alpine -d 'Search for text in the given database'
...
complete -c brew -l max-wait -d 'Wait at most this many seconds for stop to finish stopping a service'
complete -c brew -l no-wait -d 'Don[u2019]t wait for stop to finish stopping the service'
complete -c brew -l keep -d 'When stopped, don[u2019]t unregister the service from launching at login (or …'
complete -c brew -l no-installed-on-request -d 'Mark installed_formula or installed_cask as not installed on request'
complete -c brew -l custom-remote -d 'Install or change a tap with a custom remote.  Useful for mirrors'
complete -c brew -l repair -d 'Add missing symlinks to tap manpages and shell completions'
...
complete -c brew -l explain -d 'Output explanation of how to get command by installing one of the providing f…'
complete -c brew -l skip-update -d 'Skip updating the executables database if any version exists on disk, no matt…'
complete -c brew -l cache -d 'Display Homebrew[u2019]s download cache.  See also $HOMEBREW_CACHE'
complete -c brew -l caskroom -d 'Display Homebrew[u2019]s Caskroom path'
complete -c brew -l cellar -d 'Display Homebrew[u2019]s Cellar path'
complete -c brew -l env -l shell -d 'Summarise Homebrew[u2019]s build environment as a plain list'
complete -c brew -l plain -d 'Generate plain output even when piped'
complete -c brew -l prefix -d 'Display Homebrew[u2019]s install path'
complete -c brew -l unbrewed -d 'List files in Homebrew[u2019]s prefix not installed by Homebrew'
complete -c brew -l repository -l repo -d 'Display where Homebrew[u2019]s Git repository is located'
complete -c brew -l taps -d 'Display the path to Homebrewâs Taps directory'
...
complete -c brew -l signing -d 'Audit for app signatures, which are required by macOS on ARM'
complete -c brew -l changed -d 'Check files that were changed from the main branch'
complete -c brew -l fix -d 'Fix style violations automatically using RuboCop[u2019]s auto-correct feature'
...
complete -c brew -l no-all-checks -d 'Don[u2019]t try to create an all bottle or stop a no-change upload'
complete -c brew -l committer -d 'Specify a committer name and email in git[u2019]s standard author format'
complete -c brew -l root-url -d 'Use the specified URL as the root of the bottle[u2019]s URL instead of Homebr…'
complete -c brew -l root-url-using -d 'Use the specified download strategy class for downloading the bottle[u2019]s …'
complete -c brew -l no-pull-requests -d 'Do not retrieve pull requests from GitHub'
complete -c brew -l no-autobump -d 'Ignore formulae/casks in autobump list (official repositories only)'
complete -c brew -l no-fork -d 'Don[u2019]t try to fork the repository'
...
complete -c brew -l bump-synced -d 'Bump additional formulae marked as synced with the given formulae'
...
complete -c brew -l no-audit -d 'Don[u2019]t run brew audit before opening the PR'
complete -c brew -l no-style -d 'Don[u2019]t run brew style --fix before opening the PR'
...
complete -c brew -l install-dependencies -d 'Install missing dependencies required to update resources'
...
complete -c brew -l limit -d 'Maximum runtime in minutes'
complete -c brew -l state-file -d 'File for caching state.  cat [--formula] [--cask] formula|cask ['
...
complete -c brew -l organisation -d 'Specify the organisation to populate sources repositories from'
complete -c brew -l team -d 'Specify the team to populate users from'
complete -c brew -l quarter -d 'Homebrew contributions quarter to search (1-4)'
...
complete -c brew -l cabal -d 'Create a basic template for a Cabal build'
...
complete -c brew -l zig -d 'Create a basic template for a Zig build'
...
complete -c brew -s O -d 'Start remote debugging over a Unix socket'
...
complete -c brew -l linux -d 'Dispatch bottle for Linux x86_64 (using GitHub runners)'
complete -c brew -l linux-arm64 -d 'Dispatch bottle for Linux arm64 (using GitHub runners)'
complete -c brew -l linux-self-hosted -d 'Dispatch bottle for Linux x86_64 (using self-hosted runner)'
...
complete -c brew -l git-revision -d 'Search for the specified version of formula starting at revision instead of H…'
complete -c brew -l days-ago -d 'Query from the specified days ago until the present.  The default is 30 days'
complete -c brew -l install-on-request -d 'Output the number of specifically requested installations of formulae'
complete -c brew -l cask-install -d 'Output the number of installations of casks'
complete -c brew -l build-error -d 'Output the number of build errors for formulae'
complete -c brew -l os-version -d 'Output the number of events by OS name and version'
complete -c brew -l homebrew-devcmdrun-developer -d 'Output the number of devcmdrun/HOMEBREW_DEVELOPER events'
complete -c brew -l homebrew-os-arch-ci -d 'Output the number of OS/Architecture/CI events'
complete -c brew -l homebrew-prefixes -d 'Output Homebrew prefixes'
complete -c brew -l homebrew-versions -d 'Output Homebrew versions'
complete -c brew -l brew-command-run -d 'Output brew commands run'
complete -c brew -l brew-command-run-options -d 'Output brew commands run with options'
complete -c brew -l brew-test-bot-test -d 'Output brew test-bot steps run'
complete -c brew -l all-core-formulae-json -d 'Output a different JSON format containing the JSON data for all Homebrew/home…'
complete -c brew -l setup -d 'Install the necessary gems, require them and exit without running a query'
complete -c brew -l no-exit-code -d 'Exit with code 0 even if no changes were made'
...
complete -c brew -l pry -d 'Use Pry instead of IRB.  Enabled by default if $HOMEBREW_PRY is set'
...
complete -c brew -l cached -d 'Print the cached linkage values stored in $HOMEBREW_CACHE, set by a previous …'
complete -c brew -l newer-only -d 'Show the latest version only if it[u2019]s newer than the current formula or …'
complete -c brew -l resources -d 'Also check resources for formulae'
complete -c brew -l extract-plist -d 'Enable checking multiple casks with ExtractPlist strategy'
complete -c brew -l autobump -d 'Include packages that are autobumped by BrewTestBot'
...
complete -c brew -l branch -d 'Branch to use the workflow from (default: main)'
complete -c brew -l no-upload -d 'Download the bottles but don[u2019]t upload them'
...
complete -c brew -l retain-bottle-dir -d 'Does not clean up the tmp directory for the bottle so it can be used later'
complete -c brew -l artifact-pattern -d 'Download artifacts with the specified pattern (default: bottles{,_*})'
...
complete -c brew -l vernier -d 'Use vernier instead of ruby-prof (the default)'
...
complete -c brew -l only-public -d 'Only generate public API documentation'
...
complete -c brew -l no-git -d 'Don[u2019]t initialize a Git repository for the tap'
complete -c brew -l pull-label -d 'Label name for pull requests ready to be pulled (default: pr-pull)'
complete -c brew -l github-packages -d 'Upload bottles to GitHub Packages.  test [options] installed_formula ['
complete -c brew -l skip-setup -d 'Don[u2019]t check if the local system is set up correctly'
...
complete -c brew -l skip-online-checks -d 'Don[u2019]t pass --online to brew audit and skip brew livecheck'
complete -c brew -l skip-new -d 'Don[u2019]t pass --new to brew audit for new formulae'
complete -c brew -l skip-new-strict -d 'Don[u2019]t pass --strict to brew audit for new formulae'
complete -c brew -l skip-dependents -d 'Don[u2019]t test any dependents'
complete -c brew -l skip-livecheck -d 'Don[u2019]t test livecheck'
complete -c brew -l skip-recursive-dependents -d 'Only test the direct dependents'
complete -c brew -l skip-checksum-only-audit -d 'Don[u2019]t audit checksum-only changes'
complete -c brew -l skip-stable-version-audit -d 'Don[u2019]t audit the stable version'
complete -c brew -l skip-revision-audit -d 'Don[u2019]t audit the revision'
complete -c brew -l only-cleanup-before -d 'Only run the pre-cleanup step.  Needs --cleanup, except in GitHub Actions'
...
complete -c brew -l stable -d 'Only run the tap syntax checks needed on stable brew'
...
complete -c brew -l only-cleanup-after -d 'Only run the post-cleanup step.  Needs --cleanup, except in GitHub Actions'
...
complete -c brew -l tested-formulae -d 'Use these tested formulae from formulae steps for a formulae dependents step'
complete -c brew -l coverage -d 'Generate code coverage reports'
complete -c brew -l generic -d 'Run only OS-agnostic tests'
complete -c brew -l no-parallel -d 'Run tests serially'
complete -c brew -l ruby-prof -d 'Use ruby-prof to profile tests'
complete -c brew -l profile -d 'Output the n slowest tests'
...
complete -c brew -l lsp -d 'Start the Sorbet LSP server'
...
complete -c brew -l ignore-errors -d 'Continue processing even if some resources can[u2019]t be resolved'
complete -c brew -l ignore-non-pypi-packages -d 'Don[u2019]t fail if formula is not a PyPI package'
...
complete -c brew -s j -d 'Return JSON for the attestation data for each bottle'
...
complete -c brew -l install-missing -d 'Install and update formulae that are missing from the database and don[u2019]…'
complete -c brew -l max-downloads -d 'Specify a maximum number of formulae to download and update'
complete -c brew -l summary-file -d 'Output a summary of the changes to a file'
...
complete -c brew -l qlplugindir -d 'Target location for Quick Look Plugins (default: ~/Library/QuickLook)'
