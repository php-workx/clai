...
complete -c objdump -l disassembler-color -d 'Enable or disable disassembler color output.  NDENT 7. 0'
...
complete -c objdump -o mllvm -d 'Specify an argument to forward to LLVM\'s CommandLine library'
...
complete -c objdump -l pretty-pgo-analysis-map -d 'When using \\%--symbolize-operands with bb-address-map and pgo-analysis-map, p…'
...
complete -c objdump -o fbasic-block-sections -d instead
...
complete -c objdump -l traceback-table -d 'Decode traceback table in disassembly output.  Implies %--disassemble'
