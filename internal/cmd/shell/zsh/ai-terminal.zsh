...
# Generated by: ai-terminal init zsh
#
# Features:
...
#   AI_TERMINAL_AUTO_DAEMON=true     # Auto-start daemon for fast voice (default: true)
...
_ai_enter_voice_mode() {
    _AI_VOICE_MODE=true
    RPROMPT="%F{magenta}🎤 Voice mode%f"
    zle redisplay
}
...
_ai_voice_accept_line() {
    ...
    if [[ "$BUFFER" == '`'* && ${#BUFFER} -gt 1 ]]; then
        ...
        local cmd=$(ai-terminal voice "$voice_input" 2>/dev/null)
        if [[ -n "$cmd" ]]; then
            echo "→ $cmd"
            # Suggestion is cached by ai-terminal voice; Tab to accept
        fi
        zle reset-prompt
        return
    fi
...
ai() {
    ...
    ai-terminal ask --context "$recent_cmds" "$@"
}
...
_ai_ensure_daemon() {
    if [[ "$AI_TERMINAL_AUTO_DAEMON" == "true" ]]; then
        # Check if daemon is running, start if not (silently in background)
        (ai-terminal daemon start >/dev/null 2>&1 &)
    fi
}
...
ai-daemon() {
    case "$1" in
        start)
            ai-terminal daemon start
            ;;
        stop)
            ai-terminal daemon stop
            ;;
        status)
            ai-terminal daemon status
            ;;
        restart)
            ai-terminal daemon stop
            ai-terminal daemon start
...
if [[ -o interactive ]]; then
    ...
    echo -e "\033[2m🤖 AI Terminal loaded. Commands: ai-fix, ai, voice, ai-daemon | \` prefix for voice mode\033[0m"
fi
