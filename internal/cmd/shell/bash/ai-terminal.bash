# ai-terminal.bash - AI Terminal Integration for Bash
# Generated by: ai-terminal init bash
#
# Features:
...
_ai_check_voice_prefix() {
    ...
    if [[ "$cmd" == '`'* && ${#cmd} -gt 1 ]]; then
        ...
        local result=$(ai-terminal voice "$voice_input" 2>/dev/null)
        if [[ -n "$result" ]]; then
            echo "→ $result"
...
if [[ $- == *i* ]]; then
    echo -e "\033[2m🤖 AI Terminal loaded. Commands: ai-fix, ai, voice, ai-toggle, run, accept | \` prefix for voice mode\033[0m"
fi
